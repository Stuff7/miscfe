...
const nestedObject = { some: { really: { nested: { object: "here" } } } };
...
export const testCases = each(
  ...
  ([key, obj]) => ([
    it(`Saves and retrieves ${key}.`, () => {
      ...
      return getLocalItem(key) === JSON.stringify(obj);
    }),
    it(`Saves, retrieves and parses ${key}.`, () => {
      setLocalItem(key, obj);
      const localItem = getLocalItem<typeof obj>(key, null, JSON.parse);
      return !!localItem && deepCompare(localItem, obj);
    }),
    it(`Removes ${key}.`, () => {
      setLocalItem(key, obj);
      removeLocalItem(key);
      return !getLocalItem<typeof obj>(key, null, JSON.parse);
    }),
    it(`Retrieves fallback when ${key} doesn't exist.`, () => {
      ...
      return !!localItem && deepCompare(localItem, fallback);
    }),
  ]),
);
